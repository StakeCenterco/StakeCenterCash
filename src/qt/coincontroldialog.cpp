...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useSwiftX) nPayFee = max(nPayFee, CENT);
        // Allow free?
    ...
    int nDisplayUnit = BitcoinUnits::STAKEC;
    ...
    QString toolTip4 = tr("Can vary +/- %1 uSTAKEC per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show STAKEC address. In tree mode, address is not shown again for direct wallet address outputs
