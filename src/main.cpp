...
#if defined(NDEBUG)
#error "StakeCenterCash cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uSTAKEC) are considered zero fee (for relaying and mining)
...
else if (nHeight >= 1  && nHeight <= 20) { nSubsidy = 82000 * COIN; }
else if (nHeight > 20   && nHeight <= 3000) { nSubsidy = 0.1 * COIN; }
else if (nHeight > 3001   && nHeight <= 7000) { nSubsidy = 10 * COIN; }
else if (nHeight > 7000   && nHeight <= 14000) { nSubsidy = 5 * COIN; }
else if (nHeight > 14000 ) { nSubsidy = 2.5 * COIN; }
...
{
    RenameThread("StakeCenterCash-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // STAKEC: We use certain sporks during IBD, so check to see if they are
